...
# Reference existing secret created manually in AWS Secrets Manager
data "aws_secretsmanager_secret" "db_credentials" {
  name = "instagram-app-db-credentials"
}
...
resource "null_resource" "docker_build_push" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      docker build -t instagram-db-app .
      docker tag instagram-db-app:latest ${data.aws_caller_identity.current.account_id}.dkr.ecr.eu-north-1.amazonaws.com/my-app:latest
      docker push ${data.aws_caller_identity.current.account_id}.dkr.ecr.eu-north-1.amazonaws.com/my-app:latest
    EOT
  }
}
...
# Define the ECS task definition with secrets
resource "aws_ecs_task_definition" "my_task" {
  ...
  container_definitions = jsonencode([{
    name  = "ECS_Container"
    image = "${data.aws_caller_identity.current.account_id}.dkr.ecr.eu-north-1.amazonaws.com/my-app:latest"
    portMappings = [{
      containerPort = 5000
      hostPort      = 5000
      protocol      = "tcp"
    ...
    secrets = [
      {
        name      = "DB_HOST"
        valueFrom = "${data.aws_secretsmanager_secret.db_credentials.arn}:DB_HOST::"
      },
      {
        name      = "DB_USER"
        valueFrom = "${data.aws_secretsmanager_secret.db_credentials.arn}:DB_USER::"
      },
      {
        name      = "DB_PASSWORD"
        valueFrom = "${data.aws_secretsmanager_secret.db_credentials.arn}:DB_PASSWORD::"
      },
      {
        name      = "DB_NAME"
        valueFrom = "${data.aws_secretsmanager_secret.db_credentials.arn}:DB_NAME::"
      }
    ]
